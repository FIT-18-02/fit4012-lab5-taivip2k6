#ifndef STRUCTURES_H
#define STRUCTURES_H

#include <iostream>

// Bảng S-box chuẩn cho AES
const unsigned char s[256] = {
    
};

// Phép nhân Galois (GF(2^8))
unsigned char gmul(unsigned char a, unsigned char b) {
    unsigned char p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        unsigned char hi_bit = (a & 0x80);
        a <<= 1;
        if (hi_bit) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

// Các hàm bổ trợ cho MixColumns
unsigned char mul2(unsigned char a) { return gmul(a, 0x02); }
unsigned char mul3(unsigned char a) { return gmul(a, 0x03); }

// Rcon dùng cho KeyExpansion
const unsigned char Rcon[11] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };

void AddRoundKey(unsigned char state[16], unsigned char* roundKey) {
    for (int i = 0; i < 16; i++) state[i] ^= roundKey[i];
}

void SubBytes(unsigned char state[16]) {
    for (int i = 0; i < 16; i++) state[i] = s[state[i]];
}

void ShiftRows(unsigned char state[16]) {
    unsigned char tmp[16];
    tmp[0] = state[0]; tmp[4] = state[4]; tmp[8] = state[8]; tmp[12] = state[12];
    tmp[1] = state[5]; tmp[5] = state[9]; tmp[9] = state[13]; tmp[13] = state[1];
    tmp[2] = state[10]; tmp[6] = state[14]; tmp[10] = state[2]; tmp[14] = state[6];
    tmp[3] = state[15]; tmp[7] = state[3]; tmp[11] = state[7]; tmp[15] = state[11];
    for (int i = 0; i < 16; i++) state[i] = tmp[i];
}

void MixColumns(unsigned char state[16]) {
    unsigned char tmp[16];
    for (int i = 0; i < 4; i++) {
        int o = i * 4;
        tmp[o]   = mul2(state[o])   ^ mul3(state[o+1]) ^ state[o+2]     ^ state[o+3];
        tmp[o+1] = state[o]         ^ mul2(state[o+1]) ^ mul3(state[o+2]) ^ state[o+3];
        tmp[o+2] = state[o]         ^ state[o+1]       ^ mul2(state[o+2]) ^ mul3(state[o+3]);
        tmp[o+3] = mul3(state[o])   ^ state[o+1]       ^ state[o+2]       ^ mul2(state[o+3]);
    }
    for (int i = 0; i < 16; i++) state[i] = tmp[i];
}

void KeyExpansion(unsigned char* key, unsigned char* expandedKey) {
    for (int i = 0; i < 16; i++) expandedKey[i] = key[i];
    int bytesGenerated = 16;
    int rconPtr = 1;
    unsigned char temp[4];
    while (bytesGenerated < 176) {
        for (int i = 0; i < 4; i++) temp[i] = expandedKey[bytesGenerated - 4 + i];
        if (bytesGenerated % 16 == 0) {
            // RotWord
            unsigned char t = temp[0];
            temp[0] = temp[1]; temp[1] = temp[2]; temp[2] = temp[3]; temp[3] = t;
            // SubWord + Rcon
            for (int i = 0; i < 4; i++) temp[i] = s[temp[i]];
            temp[0] ^= Rcon[rconPtr++];
        }
        for (int i = 0; i < 4; i++) {
            expandedKey[bytesGenerated] = expandedKey[bytesGenerated - 16] ^ temp[i];
            bytesGenerated++;
        }
    }
}

#endif
